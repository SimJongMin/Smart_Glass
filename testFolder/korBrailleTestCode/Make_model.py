...
def Make_model(train,val):
    ...
    model_ckpt = ModelCheckpoint('E:/22-1/CapstoneDesign/Smart_Glass/testFolder/korBrailleTestCode/KorBrailleNet.h5',save_best_only=True)
    reduce_lr = ReduceLROnPlateau(patience=8,verbose=1)
    early_stop = EarlyStopping(patience=5,verbose=2,monitor='accuracy')

    entry = L.Input(shape=(36,42,3))
    ...
    history = model.fit_generator(train,validation_data=val,epochs=60,
                                  callbacks=[model_ckpt,reduce_lr,early_stop],verbose=2)                # 여기서 매번 실패한다.
                                                                                                        # 데이터가 test, valid로 분할 되는지 부터 확인하자.
